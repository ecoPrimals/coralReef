...
- **Sprint 14 (current)**: Wave 156e — capability registry drift fix, WHATS_NEXT.md fossil cleanup, SM30 latency tests (+13). Wave 156b — deep debt deduplication (ShaderInfo::compute() -220 LOC, adapter alloc fix, duplicate test removal -551 LOC). Wave 156a — test extraction (btsp.rs, types.rs), coverage tests (env_keys.rs, tolerances.rs). Wave 155j — NUCLEUS ACHIEVED on strandGate. Wave 155i — Windows cross-compilation, PTX macros, clone reduction. Wave 152 — deduplication (`require_math_arg()`, test helper consolidation). Wave 151b — BTSP client. Wave 146 — transport abstraction.
