...
**Version**: 0.2.0 — Sprint 14 / Wave 77 (Deep debt sweep complete: smart refactoring of last >800L production files — expr_eval image extraction, hyperbolic math consolidation. Full audit clean: all deps pure Rust, no unsafe in prod, no hardcoded primal names, no production mocks. 3303 tests, zero clippy warnings)
...
| Tests | A+ | 3303 passing, 0 failed, IR idempotency (WGSL roundtrip + SPIR-V roundtrip + multi-backend determinism), `primal.announce` payload schema, `--socket` CLI override, tarpc Unix roundtrip, IPC chaos/fault tests, BTSP Phase 3 AEAD crypto tests, Compute Trio wire contract, PTX emitter SM120, HMMA GEMM, RayQuery, texture format coverage, inverse trig, geometry math, bit manipulation, texture queries, hyperbolic trig, float decomposition, bit scan, adapter-aware arch inference, subgroupBallot copy-prop regression, sovereign SPIR-V emission, SM120 membar.sys barrier, math pack/unpack builtins (10 variants), matrix transpose/determinant/inverse, multi-entry-point module hardening, SPIR-V version targeting, provenance hash determinism |
...
**Theme**: Upstream primal audit (primalSpring Passes 12-14) — three sentinel escalation items resolved. `naga::Module` direct ingest API (skip text→parse for in-process callers), compile deadline on all IPC handlers (120s default, `CORALREEF_COMPILE_TIMEOUT_SECS`), Volta+/Blackwell cold init wired to attempt PIO falcon boot when firmware available. 4790 tests (+6).
