...
**Version**: 0.2.0 — Sprint 14 / Wave 157d (Integer subgroup scan/reduce correctness fix: `emit_scan_via_shfl`/`emit_reduce_via_shfl` now dispatch on type+op instead of hardcoding `OpFAdd`. Silicon fold response AAR filed. Self-audit: 18/18 RPC methods match `capability_registry.toml`. G68 16/16 prod-clean. Zero P0s. 3,702 tests. Zero unsafe, zero clippy warnings on Linux + Windows.)
