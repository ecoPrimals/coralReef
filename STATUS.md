...
**Version**: 0.2.0 — Sprint 14 / Wave 157d (GEMM tiling Phase 1: fully tiled `mma.sync.aligned` kernels with `%tid`/`%ctaid` thread mapping, per-thread MMA fragment addressing, strided global loads, row-major C store. f16/f16f32/TF32 all tiled. M/N alignment validation. Integer subgroup scan/reduce fix. Silicon fold AAR. Self-audit: 18/18 RPC methods match `capability_registry.toml`. G68 16/16 prod-clean. Zero P0s. 3,712 tests. Zero unsafe, zero clippy warnings on Linux + Windows.)
...
| `cargo test --workspace` | PASS (3525 total: 3519 passing, 0 failed, 6 ignored hardware-gated) |
