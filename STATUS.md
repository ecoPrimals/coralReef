...
**Version**: 0.2.0 — Sprint 14 / Wave 151b (BTSP client handshake: `ClientHello → ServerHello → ChallengeResponse → HandshakeComplete` per `BTSP_PROTOCOL_STANDARD` v1.0, `crypto.sign` provenance signing via BTSP in production mode. 3700 tests, 84% line coverage, zero clippy warnings, zero unsafe.)
...
| `cargo test --workspace` | PASS (3700 total: 3696 passing, 0 failed, 4 ignored hardware-gated) |
