...
pub(super) fn kepler_post_done_boot_fecs(
    ...
    for delay_us in [10, 50, 100, 500, 1000, 5000, 10000u64] {
        ...
        let fec_cpuctl = rd(FECS + 0x100);
        let fec_prog_cnt = rd(FECS + 0x0A8);
        let fec_idle = rd(FECS + 0x04C);
        let fec_exci = rd(FECS + 0x018);
        let fec_sp = rd(FECS + 0x0A0);
        let mb = rd(0x40_9800);
        tracing::info!(
            us = delay_us,
            cpuctl = format_args!("{fec_cpuctl:#010x}"),
            pc = format_args!("{fec_prog_cnt:#010x}"),
            idle = format_args!("{fec_idle:#010x}"),
            exci = format_args!("{fec_exci:#010x}"),
            sp = format_args!("{fec_sp:#010x}"),
