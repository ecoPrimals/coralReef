...
pub(crate) fn read_boot0(resource0_path: &str) -> Option<u32> {
    ...
    let mut file = std::fs::OpenOptions::new().read(true).open(resource0_path).ok()?;
...
fn bind_vfio(
    ...
    // Kill reset_method IMMEDIATELY after vfio-pci binds. The kernel
    // re-enables "bus" when the driver changes; if we wait until after
    // settle, vfio-pci's first device-open fires a PCI bus reset that
    // wipes all GPU state (falcon IMEM, GR context, HBM2 training).
    let reset_path = linux_paths::sysfs_pci_device_file(bdf, "reset_method");
...
fn pci_remove_rescan(bdf: &str, target_driver: Option<&str>) -> Result<(), String> {
    sysfs::pci_remove_rescan_targeted(bdf, target_driver)
}
...
fn bind_native(
    ...
    // Set driver_override to the TARGET driver (not "\n"/clear).
    // This ensures the kernel's autoprobe and any fallback PCI rescan
    // will bind the correct driver, even if vfio-pci.ids or other
    // built-in match tables would otherwise reclaim the device.
    sysfs::sysfs_write(
        &linux_paths::sysfs_pci_device_file(bdf, "driver_override"),
        target,
    )?;
