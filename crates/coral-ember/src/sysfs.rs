...
#[path = "sysfs_pci_topology.rs"]
mod sysfs_pci_topology;
pub use sysfs_pci_topology::*;
...
///
/// Delegates to `coral_driver::linux_paths::isolated_sysfs_write` which forks
/// a child process that writes directly via `std::fs::write`. No shell involved.
///
/// Process isolation is critical because:
...
fn guarded_sysfs_write(path: &str, value: &str, timeout: Duration) -> Result<(), String> {
    coral_driver::linux_paths::isolated_sysfs_write(path, value, timeout).map_err(|e| {
        tracing::error!(path, value, error = %e, "guarded sysfs write failed");
        e
    })
}
