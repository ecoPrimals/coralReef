...
pub(crate) fn err_if_pm_cycle_d3cold(bdf: &str, after_power_state: &str) -> Result<(), SysfsError> {
    if after_power_state == "D3cold" {
        return Err(SysfsError::PciReset {
            bdf: bdf.to_string(),
            reason: "PM power cycle resulted in D3cold".to_string(),
        });
    }
    Ok(())
}
...
pub fn pci_bridge_reset(bdf: &str) -> Result<(), SysfsError> {
    let bridge_bdf = find_parent_bridge(bdf).ok_or_else(|| SysfsError::PciReset {
        bdf: bdf.to_string(),
        reason: "cannot find parent PCI bridge for bridge-level SBR".to_string(),
    })?;
    ...
    if !std::path::Path::new(&bridge_reset).exists() {
        return Err(SysfsError::PciReset {
            bdf: bdf.to_string(),
            reason: format!("parent bridge {bridge_bdf} has no reset file"),
        });
    }
...
pub fn pci_remove_rescan_targeted(bdf: &str, target_driver: Option<&str>) -> Result<(), SysfsError> {
    pin_bridge_power(bdf);
    pin_power(bdf);
...
fn pci_remove_rescan_inner(bdf: &str, target_driver: Option<&str>) -> Result<(), SysfsError> {
    ...
    Err(SysfsError::PciReset {
        bdf: bdf.to_string(),
        reason: "device did not re-appear after PCI rescan".to_string(),
    })
}
...
mod tests {
    ...
    fn err_if_pm_cycle_d3cold_rejects_d3cold() {
        ...
        assert!(msg.contains(bdf));
        assert!(msg.contains("D3cold"));
    }
