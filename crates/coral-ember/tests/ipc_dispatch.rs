...
fn dispatch_invalid_json_parse_error() {
    ...
    handle_client(&server, &mut held, Instant::now()).expect("handler");
...
fn dispatch_ember_status_uptime() {
    ...
    let mut held = HashMap::new();
    handle_client(&server, &mut held, started).expect("handler");
    let v = drain_json_line(&mut client);
...
fn dispatch_ember_release_missing_bdf_errors() {
    ...
    client.write_all(b"\n").expect("newline");
    let mut held = HashMap::new();
    let err = handle_client(&server, &mut held, Instant::now()).expect_err("missing bdf");
...
fn dispatch_ember_swap_missing_target() {
    ...
    client.write_all(b"\n").expect("newline");
    let mut held = HashMap::new();
    let err = handle_client(&server, &mut held, Instant::now()).expect_err("missing target");
...
fn dispatch_ember_vfio_fds_with_hardware() {
    ...
    held.insert(bdf.clone(), HeldDevice { bdf, device });
    handle_client(&server, &mut held, Instant::now()).expect("handler");
}
