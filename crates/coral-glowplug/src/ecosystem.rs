...
//! Ecosystem discovery file management for coral-glowplug.
//!
//! Writes a JSON discovery file to `$XDG_RUNTIME_DIR/biomeos/` on startup
//! so that other primals can discover coral-glowplug's capabilities and
//! socket address at runtime. Follows the same schema and directory
//! convention as the wateringHole `UNIVERSAL_IPC_STANDARD_V3`.
...
/// Write a discovery JSON file advertising coral-glowplug's capabilities and socket bind.
///
/// Called after the Unix socket is bound. The file is placed in the shared
/// ecosystem directory so peers can discover glowplug without hardcoded paths.
pub fn write_discovery_file(socket_bind: &str, device_count: usize) {
...
mod tests {
    ...
    fn discovery_json_schema() {
        ...
        let discovery = serde_json::json!({
            ...
            "version": env!("CARGO_PKG_VERSION"),
            "pid": std::process::id(),
            "provides": ["gpu.lifecycle", "gpu.health", "gpu.dispatch"],
            "requires": ["gpu.vfio.hold"],
            "transports": {
                "jsonrpc_unix": { "bind": "unix:///tmp/test.sock" },
            },
            "metadata": { "device_count": 3 },
        });
        ...
        assert_eq!(content["metadata"]["device_count"], 3);
        assert!(content["provides"]
            .as_array()
            .unwrap()
            .iter()
            .any(|v| v == "gpu.lifecycle"));
