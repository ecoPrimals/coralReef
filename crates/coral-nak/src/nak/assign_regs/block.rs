...
impl AssignRegsBlock {
    pub(super) fn new(num_regs: &PerRegFile<u32>, pcopy_tmp_gprs: u8) -> AssignRegsBlock {
        AssignRegsBlock {
            ra: PerRegFile::new_with(|file| RegAllocator::new(file, num_regs[file])),
            pcopy_tmp_gprs,
            live_in: Vec::new(),
    ...
    pub(super) fn second_pass(&self, target: &AssignRegsBlock, b: &mut BasicBlock) {
        let mut pcopy = OpParCopy::new();
        pcopy.tmp = self.pcopy_tmp();
