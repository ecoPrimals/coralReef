...
/// Tessellation spacing constants (matches NVIDIA encoding).
pub const NAK_TS_SPACING_INTEGER: u8 = 0;
pub const NAK_TS_SPACING_FRACT_ODD: u8 = 1;
pub const NAK_TS_SPACING_FRACT_EVEN: u8 = 2;

/// Transform feedback (XFB) info for vertex/geometry shader output.
#[derive(Debug, Default)]
pub struct NakXfbInfo {
    /// Number of XFB buffers used.
    pub num_buffers: u8,
...
fn can_isbe_space_sharing_be_enabled(f: &Function) -> bool {
    ...
    {
        use coral_nak_stubs::dataflow::ForwardDataflow;
