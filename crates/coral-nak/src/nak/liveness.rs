...
use coral_nak_stubs::bitset::BitSet;
use coral_nak_stubs::dataflow::{BackwardDataflow, BackwardDataflowBi};
        ...
        for (bb_idx, bb) in f.blocks.iter().enumerate() {
            ...
            for (ip, instr) in bb.instrs.iter().enumerate() {
                ...
                if let Op::RegOut(reg_out) = &instr.op {
                    ...
                    max_live[RegFile::GPR] = max(max_live[RegFile::GPR], num_gprs_out);
                }
            }
...
impl NextUseLiveness {
    pub fn for_function(func: &Function, files: &RegFileSet) -> NextUseLiveness {
        ...
        let mut live_out: Vec<FxHashMap<SSAValue, usize>> = (0..func.blocks.len())
            ...
            .collect();
        {
            use coral_nak_stubs::dataflow::BackwardDataflowBi;
