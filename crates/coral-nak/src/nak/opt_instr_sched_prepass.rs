...
fn next_occupancy_cliff_with_reserved(sm: &ShaderModelInfo, gprs: i32, reserved: i32) -> i32 {
...
mod net_live {
    use crate::nak::ir::*;
    use crate::nak::liveness::LiveSet;
    use rustc_hash::FxHashMap;
    use std::ops::Index;
        ...
        pub(super) fn new(instrs: &[Instr], live_out: &LiveSet) -> Self {
            ...
            let mut counts: Vec<InstrCount> = instrs
                ...
                .map(|(instr_idx, instr)| {
                    ...
                    let net = PerRegFile::new_with(|f| use_set.count(f).try_into().unwrap());
                    InstrCount {
                        net,
...
fn calc_score_part(mut p: PerRegFile<i32>, max_regs: PerRegFile<i32>) -> (i32, i32) {
...
impl<'a> GenerateOrder<'a> {
    fn new(max_regs: PerRegFile<i32>, instrs: &'a [Instr], live_out: &LiveSet) -> Self {
...
fn sched_buffer(
    ...
    let (mut new_order, live_in_count2) = GenerateOrder::new(max_regs, instrs, live_out)
    ...
    fn thresholds(
        ...
        match self {
            ...
            ScheduleType::Spill => ScheduleThresholds {
                heuristic_threshold: max_regs[RegFile::GPR] - SW_RESERVED_GPRS_SPILL - TARGET_FREE,
...
impl Function {
    pub fn opt_instr_sched_prepass(&mut self, sm: &ShaderModelInfo, max_regs: PerRegFile<i32>) {
        ...
        for block_idx in 0..self.blocks.len() {
            ...
            for (ip, instr) in std::mem::take(&mut block.instrs).into_iter().enumerate() {
                ...
                {
                    let live_count = PerRegFile::new_with(|f| live_count[f].try_into().unwrap());
        ...
        let mut schedule_types =
            get_schedule_types(sm, max_regs, min_gpr_target, max_gpr_target, reserved_gprs);
...
impl Shader<'_> {
    ...
    pub fn opt_instr_sched_prepass(&mut self) {
        ...
        let mut max_regs = PerRegFile::<i32>::new_with(|f| self.sm.num_regs(f).try_into().unwrap());
        if let ShaderStageInfo::Compute(cs_info) = &self.info.stage {
            max_regs[RegFile::GPR] = min(
                max_regs[RegFile::GPR],
                (gpr_limit_from_local_size(&cs_info.local_size) - self.sm.hw_reserved_gprs())
                    .try_into()
                    .unwrap(),
            );
        }
