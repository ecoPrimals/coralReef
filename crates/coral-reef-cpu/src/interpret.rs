...
impl BindingMemory {
    ...
    fn read_f32(&self, group: u32, binding: u32, byte_offset: usize) -> Result<f32, CpuError> {
        let buf = self
            .buffers
            .get(&(group, binding))
...
impl Value {
    fn as_f64(&self) -> Result<f64, CpuError> {
        match self {
            Self::F64(v) => Ok(*v),
    ...
    fn as_u32(&self) -> Result<u32, CpuError> {
        match self {
            Self::U32(v) => Ok(*v),
    ...
    fn as_i32(&self) -> Result<i32, CpuError> {
        match self {
            Self::I32(v) => Ok(*v),
    ...
    fn as_bool(&self) -> Result<bool, CpuError> {
        match self {
            Self::Bool(v) => Ok(*v),
            Self::U32(v) => Ok(*v != 0),
            Self::I32(v) => Ok(*v != 0),
...
struct InterpreterState<'a> {
    ...
    memory: &'a mut BindingMemory,
    locals: HashMap<naga::Handle<naga::LocalVariable>, Value>,
    expressions: HashMap<naga::Handle<naga::Expression>, Value>,
}
...
fn eval_function_argument(state: &InterpreterState<'_>, index: usize) -> Result<Value, CpuError> {
    let arg = state
        .function
        .arguments
        .get(index)
...
fn eval_literal(lit: &naga::Literal) -> Value {
    match *lit {
...
fn eval_binary(op: naga::BinaryOperator, left: &Value, right: &Value) -> Result<Value, CpuError> {
...
fn eval_unary(op: naga::UnaryOperator, val: &Value) -> Result<Value, CpuError> {
...
fn eval_cast(val: &Value, kind: naga::ScalarKind) -> Result<Value, CpuError> {
    match kind {
...
fn default_value_for_type(module: &naga::Module, ty: naga::Handle<naga::Type>) -> Value {
