...
impl Instr {
    ...
    pub fn ssa_uses(&self) -> impl Iterator<Item = &super::SSAValue> {
        self.srcs()
            ...
            .chain(self.pred.predicate.iter_ssa())
    }
...
mod tests {
    use super::*;
    use crate::codegen::ir::OpNop;

    #[test]
    fn test_instr_new_default_pred() {
        let instr = Instr::new(OpNop { label: None });
        assert!(instr.pred.is_true());
    }

    #[test]
    fn test_instr_deps_new() {
        let deps = InstrDeps::new();
        assert_eq!(deps.delay, 0);
        assert!(!deps.yld);
        assert!(deps.rd_bar().is_none());
        assert!(deps.wr_bar().is_none());
    }

    #[test]
    fn test_instr_deps_set_delay() {
        let mut deps = InstrDeps::new();
        deps.set_delay(5);
        assert_eq!(deps.delay, 5);
    }

    #[test]
    fn test_instr_deps_barriers() {
        let mut deps = InstrDeps::new();
        deps.set_rd_bar(0);
        deps.set_wr_bar(1);
        assert_eq!(deps.rd_bar(), Some(0));
        assert_eq!(deps.wr_bar(), Some(1));
    }

    #[test]
    fn test_instr_dsts_srcs_empty_for_nop() {
        let instr = Instr::new(OpNop { label: None });
    ...
    #[test]
    fn test_instr_nop_cannot_eliminate() {
        // Nop is explicitly excluded from elimination (e.g. used for alignment/labels)
        let instr = Instr::new(OpNop { label: None });
        assert!(!instr.can_eliminate());
    }
}
