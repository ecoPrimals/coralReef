    ...
    match &src.reference {
        ...
        SrcRef::SSA(ssa) => {
            ...
            match ssa[0].file() {
                RegFile::Pred => false,
                RegFile::UPred => true,
                _ => panic!("ICE: Not a predicate source"),
            }
        }
...
pub trait LegalizeBuildHelpers: SSABuilder {
    ...
    fn copy_pred_ssa_if_uniform(&mut self, ssa: &mut SSAValue) {
        match ssa.file() {
            ...
            _ => panic!("ICE: Not a predicate value"),
        }
    }
    ...
    fn copy_pred_if_upred(&mut self, pred: &mut Pred) {
        match &mut pred.predicate {
            ...
            PredRef::Reg(_) => panic!("ICE: Not in SSA form"),
        }
    }
    ...
    fn copy_alu_src(&mut self, src: &mut Src, reg_file: RegFile, src_type: SrcType) {
        let val = match src_type {
            ...
            _ => panic!("ICE: Unknown source type"),
        };
        ...
        if val.comps() == 1 {
            ...
            match old_src_ref {
                ...
                _ => panic!("ICE: Invalid 64-bit SrcRef"),
            }
        }
    }
    ...
    fn copy_alu_src_and_lower_fmod(&mut self, src: &mut Src, reg_file: RegFile, src_type: SrcType) {
        match src_type {
            ...
            _ => panic!("ICE: Invalid ffabs srouce type"),
        }
    }
...
fn legalize_instr(
    ...
    match &instr.op {
        ...
        Op::Swap(_) | Op::ParCopy(_) => {
            ...
            panic!("ICE: Unsupported instruction");
        }
        _ => (),
    }
