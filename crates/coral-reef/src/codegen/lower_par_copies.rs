...
impl CopyGraph {
        ...
        if self.nodes[node_idx].src < 0 {
            ...
            Some(
                self.nodes[node_idx]
                    .src
                    .try_into()
                    .expect("src >= 0 here; isize fits in usize"),
            )
        }
    }
    ...
    fn add_edge(&mut self, dst_idx: usize, src_idx: usize) {
        ...
        self.nodes[dst_idx].src = src_idx.try_into().expect("node indices fit in isize");
...
fn lower_par_copy(pc: OpParCopy, sm: &dyn ShaderModel) -> MappedInstrs {
    ...
    for (i, (dst, _)) in pc.dsts_srcs.iter().enumerate() {
        ...
        let reg = dst
            .as_reg()
            .expect("OpParCopy dsts are Reg; run after register allocation");
    ...
    while let Some(dst_idx) = ready.pop() {
        if let Some(src_idx) = graph.src(dst_idx) {
            let dst = *vals[dst_idx]
                .as_reg()
                .expect("vals[dst_idx] is from dsts_srcs; dsts are Reg");
    ...
    for i in 0..pc.dsts_srcs.len() {
        ...
        let file = vals[i]
            .as_reg()
            .expect("vals[i] is dst or src; both are Reg in par copy")
            .file();
        if cycle_use_swap(&pc, file) {
            loop {
                let j = graph
                    .src(i)
                    .expect("graph.src(i) is Some; we checked above and we're in a cycle");
                // We're part of a cycle so j also has a source
                let k = graph
                    .src(j)
                    .expect("node j in cycle has src; cycles have in-degree 1");
                let j_reg = vals[j].as_reg().expect("vals[j] is Reg; from dsts_srcs");
                let k_reg = vals[k].as_reg().expect("vals[k] is Reg; from dsts_srcs");
                b.swap(*j_reg, *k_reg);
                graph.del_edge(i, j);
                graph.del_edge(j, k);
                if i == k {
            ...
            let mut p = i;
            let mut p_reg = *vals[p].as_reg().expect("vals[p] is Reg; p is dst index");
            b.copy_to(tmp.into(), p_reg.into());

            loop {
                let j = graph.src(p).expect("node p in cycle has src");
                if j == i {
                    break;
                }
                ...
                let j_reg = *vals[j]
                    .as_reg()
                    .expect("vals[j] is Reg; j is dst index in cycle");
                ...
                Op::ParCopy(pc) => {
                    ...
                    if DEBUG.annotate() {
                        instrs.push(Instr::new(OpAnnotate {
                            annotation: "par_copy lowered by lower_par_copy".into(),
                        }));
                    }
