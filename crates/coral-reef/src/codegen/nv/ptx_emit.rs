...
impl PtxVal {
    ...
    fn component(&self, idx: usize) -> &PtxVal {
        match self {
            Self::Vec(v) => &v[idx],
            _ if idx == 0 => self,
...
impl<'a> PtxEmitter<'a> {
    ...
    fn resolve_expr_type_handle(&self, h: naga::Handle<naga::Expression>) -> naga::Handle<naga::Type> {
        ...
        match *expr {
            ...
            naga::Expression::Literal(ref lit) => self.scalar_type_handle(super::super::naga_translate::lit_scalar(lit)),
            ...
            _ => self.module.types.iter().next().map(|(h, _)| h).unwrap_or_else(|| {
        ...
        match builtin {
            naga::BuiltIn::GlobalInvocationId => {
                ...
                for (axis, dim_label) in ["x", "y", "z"].iter().enumerate() {
                    ...
                    writeln!(self.body, "    mov.u32 {}, %tid.{dim_label};", tid.fmt_operand())
                        .unwrap();
            ...
            naga::BuiltIn::LocalInvocationId => {
                ...
                for dim_label in ["x", "y", "z"] {
                    ...
                    writeln!(self.body, "    mov.u32 {}, %tid.{dim_label};", r.fmt_operand())
                        .unwrap();
                    components.push(r);
                }
                Ok(PtxVal::Vec(components))
    ...
    fn emit_stmt(&mut self, stmt: &naga::Statement) -> Result<(), CompileError> {
        match *stmt {
            ...
            naga::Statement::Switch { .. } => Err(CompileError::NotImplemented(
                "PTX switch statement".into(),
            )),
            naga::Statement::Kill => {
    ...
    fn emit_store(
        ...
        if let Some(lv_comp) = self.expr_is_local_var_component(pointer) {
            ...
            if let Some(local) = self.locals.get(&lv_handle).cloned() {
                if let PtxVal::Vec(ref components) = local {
    ...
    fn eval_expr(
        ...
        let result = match expr {
            ...
            naga::Expression::Math {
                ...
                let b = arg1.map(|h| self.eval_expr(h)).transpose()?;
                let c = arg2.map(|h| self.eval_expr(h)).transpose()?;
                self.eval_math(fun, &a, b.as_ref(), c.as_ref(), arg)
            }
            ...
            naga::Expression::Compose { ty: _, ref components } => {
    ...
    fn eval_binary(
        ...
        if let (PtxVal::Vec(lv), PtxVal::Vec(rv)) = (left, right) {
            ...
            for (l, r) in lv.iter().zip(rv.iter()) {
                results.push(self.eval_binary_scalar(op, l, r, scalar, ts)?);
            }
    ...
    fn eval_binary_scalar(
        ...
        match op {
            BO::Add | BO::Subtract | BO::Multiply | BO::Divide | BO::Modulo => {
                ...
                let suffix = if matches!(op, BO::Divide) && is_float {
                    ts
                } else if matches!(op, BO::Multiply) && is_float {
                    ts
                } else {
                    ts
                };
            ...
            BO::Equal | BO::NotEqual | BO::Less | BO::LessEqual | BO::Greater
            | BO::GreaterEqual => {
                let pred = self.alloc_pred();
                let cmp = match op {
                    BO::Equal => "eq",
    ...
    fn eval_math(
        ...
        match fun {
            ...
            MF::Min => {
                let b = arg1.ok_or(CompileError::NotImplemented("min without arg1".into()))?;
                let dst = self.alloc_for_scalar(scalar);
                writeln!(
                    self.body,
                    "    min.{ts} {}, {}, {};",
            ...
            MF::Max => {
                let b = arg1.ok_or(CompileError::NotImplemented("max without arg1".into()))?;
                let dst = self.alloc_for_scalar(scalar);
                writeln!(
                    self.body,
                    "    max.{ts} {}, {}, {};",
            ...
            MF::Clamp => {
                let lo = arg1.ok_or(CompileError::NotImplemented("clamp without arg1".into()))?;
                let hi = arg2.ok_or(CompileError::NotImplemented("clamp without arg2".into()))?;
            ...
            MF::Fma => {
                let b = arg1.ok_or(CompileError::NotImplemented("fma without arg1".into()))?;
                let c = arg2.ok_or(CompileError::NotImplemented("fma without arg2".into()))?;
                let dst = self.alloc_for_scalar(scalar);
                writeln!(
                    self.body,
                    "    fma.rn.{ts} {}, {}, {}, {};",
