...
pub(super) fn emit_f16_gemm(
    ...
    writeln_ptx!(
        ptx,
        "    and.u32 %r15, %r13, 3;       // tid_in_group (0..3)"
    );
    writeln_ptx!(ptx);
    ...
    writeln_ptx!(
        ptx,
        "    mul.lo.u32 %r16, %r12, 16;   // m_start = ctaid.y * 16"
    );
    writeln_ptx!(
        ptx,
        "    mul.lo.u32 %r17, %r11, 8;    // n_start = ctaid.x * 8"
    );
    writeln_ptx!(ptx);
    ...
    writeln_ptx!(
        ptx,
        "    add.u32 %r18, %r16, %r14;    // a_row0 = m_start + group_id"
    );
    writeln_ptx!(
        ptx,
    ...
    writeln_ptx!(
        ptx,
        "    shl.b32 %r20, %r15, 2;       // tid_in_group * 4 (byte offset for 2 f16)"
    );
    writeln_ptx!(
        ptx,
        "    add.u32 %r21, %r19, %r20;    // a_row0_byte_offset"
    );
    ...
    writeln_ptx!(
        ptx,
        "    add.u32 %r22, %r18, 8;       // a_row1 = a_row0 + 8"
    );
    writeln_ptx!(ptx, "    mul.lo.u32 %r23, %r22, {k_times_2};");
    ...
    writeln_ptx!(
        ptx,
        "    add.u32 %r25, %r17, %r14;    // b_col = n_start + group_id"
    );
    writeln_ptx!(
        ptx,
    ...
    for ki in 0..k_iters {
        ...
        writeln_ptx!(
            ptx,
            "    // --- K iteration {ki} (k_base={kb}) ---",
            kb = ki * tile_k
        );
        writeln_ptx!(ptx, "    ld.global.b32 %r4, [%rd3+{k_byte_off}];");
        writeln_ptx!(
            ptx,
            "    ld.global.b32 %r5, [%rd3+{off}];",
            off = k_byte_off + 16
        );
        writeln_ptx!(ptx, "    ld.global.b32 %r6, [%rd4+{k_byte_off}];");
        writeln_ptx!(
            ptx,
        ...
        writeln_ptx!(ptx, "    ld.global.b32 %r8, [%rd5+{k_byte_off}];");
        writeln_ptx!(
            ptx,
    ...
    writeln_ptx!(
        ptx,
        "    add.u64 %rd6, %rd2, %rd6;    // C base addr for this thread"
    );
    writeln_ptx!(ptx);
    ...
    if f32_acc {
        ...
        writeln_ptx!(
            ptx,
            "    st.global.b32 [%rd6], %r0;              // C[row0, col0:col0+1]"
        );
        writeln_ptx!(
            ptx,
...
pub(super) fn emit_tf32_gemm(shape: GemmShape, sm: u8) -> Result<String, CompileError> {
    ...
    writeln_ptx!(
        ptx,
        "    // Precompute A fragment row base addresses (tf32, 4 bytes/elem)"
    );
    writeln_ptx!(
        ptx,
        "    add.u32 %r18, %r16, %r14;    // a_row0 = m_start + group_id"
    ...
    writeln_ptx!(
        ptx,
        "    shl.b32 %r20, %r15, 2;       // tid_in_group * 4 (1 f32 element)"
    );
    ...
    writeln_ptx!(ptx, "    mul.lo.u32 %r23, %r22, {k_times_4};");
    writeln_ptx!(ptx, "    add.u32 %r24, %r23, %r20;");
    writeln_ptx!(ptx, "    cvt.u64.u32 %rd4, %r24;");
    ...
    writeln_ptx!(
        ptx,
        "    // Precompute B fragment column base address (tf32)"
    );
    writeln_ptx!(
        ptx,
        "    add.u32 %r25, %r17, %r14;    // b_col = n_start + group_id"
    );
    writeln_ptx!(ptx, "    mul.lo.u32 %r26, %r25, {k_times_4};");
