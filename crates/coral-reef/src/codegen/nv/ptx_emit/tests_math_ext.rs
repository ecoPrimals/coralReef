...
";
        ...
        let compiled = result.unwrap();
        let ptx = String::from_utf8_lossy(&compiled.binary);
        assert!(ptx.contains("sin.approx"), "should use sin.approx: {ptx:.600}");
        assert!(ptx.contains("cos.approx"), "should use cos.approx: {ptx:.600}");
