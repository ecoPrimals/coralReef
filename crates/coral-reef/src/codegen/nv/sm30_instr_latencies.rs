...
use crate::codegen::ir::{Instr, InstrDeps, Label, Op, OpNop, Shader};
...
pub trait KeplerInstructionEncoder {
    ...
    fn encode_instr(&self, instr: &Instr, labels: &FxHashMap<Label, usize>, encoded: &mut Vec<u32>);
    ...
    E: KeplerInstructionEncoder,
{
    ...
    let mut instr_iter = func.blocks.iter().flat_map(|b| b.instrs.iter()).peekable();
