...
use super::sm80_instr_latencies::SM80Latency;
...
impl ShaderModel70 {
    ...
    fn instr_latency(&self, op: &Op, dst_idx: usize) -> u32 {
        ...
        // Volta (SM70/SM72) has a real latency table in sm70_instr_latencies.rs;
        // fall back to conservative estimates only for SM73 (Turing-lite) where
        // we lack documented latency data.
        let (gpr_latency, pred_latency) = if self.sm < 80 && !self.is_volta() {
            match op {
...
impl ShaderModel for ShaderModel70 {
    ...
    fn reg_count(&self, file: RegFile) -> u32 {
        match file {
            RegFile::GPR => 255 - self.hw_reserved_gpr_count(),
            RegFile::UGPR => {
    ...
    fn op_needs_scoreboard(&self, op: &Op) -> bool {
        ...
        if self.is_blackwell() {
            ...
            SM70Latency::needs_scoreboards(op)
        } else {
    ...
    fn raw_latency(&self, write: &Op, dst_idx: usize, read: &Op, src_idx: usize) -> u32 {
        if self.is_blackwell() {
            ...
            SM70Latency::raw(write, dst_idx, Some(read), src_idx)
        } else {
    ...
    fn war_latency(&self, read: &Op, src_idx: usize, write: &Op, dst_idx: usize) -> u32 {
        if self.is_blackwell() {
            ...
            SM70Latency::war(read, src_idx, write, dst_idx)
        } else {
            // Conservative guess for SM73 (Turing-lite) without documented data.
            4
        }
    }
    ...
    fn waw_latency(
        ...
        if self.is_blackwell() {
            ...
            SM70Latency::waw(a, a_dst_idx, b, b_dst_idx, a_has_pred)
        } else {
            // Conservative for SM73 without documented data.
    ...
    fn latency_upper_bound(&self) -> u32 {
        if self.is_blackwell() || self.is_ampere() || self.is_ada() {
            ...
            // SM70 max fixed latency: ~15cy (predicate reads on FP64 writes).
            // Shared-memory and global loads are decoupled (scoreboard), so
            // their latency does not appear here.
            15
        } else {
            15
        }
    ...
    fn worst_latency(&self, write: &Op, dst_idx: usize) -> u32 {
        if self.is_blackwell() {
            ...
            SM70Latency::raw(write, dst_idx, None, 0)
        } else {
            self.instr_latency(write, dst_idx)
        }
    }
