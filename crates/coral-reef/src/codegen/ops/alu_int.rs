...
//! Integer ALU operation encoding — IAdd3, IMad, IMnMx, Lop2, Lop3, PLop3,
//! Shl, Shr, Shf, Sel, PopC, BRev, Flo, Bfe, BMsk.
//!
...
// ---- PLop3 (SOP2: S_AND_B32 / S_OR_B32 / S_XOR_B32 / etc.) ----
//
...
impl EncodeOp<AmdOpEncoder<'_>> for OpPLop3 {
    ...
    fn encode(&self, _e: &mut AmdOpEncoder<'_>) -> Result<Vec<u32>, CompileError> {
        ...
        // 1-input patterns: NOT(x), NOT(y), passthrough
        ...
        if lut == 0x00 {
            return Ok(Rdna2Encoder::encode_sop1(
                isa::sop1::S_MOV_B32,
                dst,
                AmdRegRef::sgpr(128), // inline constant 0
            ));
        }
        if lut == 0xFF {
            return Ok(Rdna2Encoder::encode_sop1(
                isa::sop1::S_MOV_B32,
                dst,
                AmdRegRef::sgpr(193), // inline constant -1
            ));
        }

        // 2-input patterns: decompose to SOP2
        let ssrc0 = plop3_src_to_ssrc(&self.srcs[0])?;
        let ssrc1 = plop3_src_to_ssrc(&self.srcs[1])?;
        ...
        let opcode = if lut == and_lut {
            ...
            // ~x & y → swap operands: S_ANDN2(y, x) = y & ~x
