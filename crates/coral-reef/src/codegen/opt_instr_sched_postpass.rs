...
use super::reg_tracker::RegTracker;
use std::cmp::Reverse;
use std::cmp::max;
...
fn generate_dep_graph(sm: &dyn ShaderModel, instrs: &[Instr]) -> DepGraph {
    ...
    for ip in (0..instrs.len()).rev() {
        ...
        uses.for_each_instr_src_mut(instr, |i, u| {
            if let Some((w_ip, w_dst_idx)) = u.write {
                let mut latency = sm.war_latency(&instr.op, i, &instrs[w_ip].op, w_dst_idx);
                if sm.op_needs_scoreboard(&instr.op) {
                    // Barriers take two cycles to become active
                    latency = max(latency, 2);
                }
...
fn generate_order(g: &mut DepGraph, init_ready_list: Vec<usize>) -> (Vec<usize>, u64) {
    ...
    loop {
        ...
        for edge in outgoing_edges.into_iter() {
            ...
            if dep_instr.use_count == 0 {
                future_ready_instrs.push(FutureReadyInstr::new(g, edge.head_idx));
            }
        }
    }
...
impl Function {
    pub fn opt_instr_sched_postpass(&mut self, sm: &dyn ShaderModel) -> u64 {
        let mut static_cycle_count = 0u64;
        for i in 0..self.blocks.len() {
            ...
            static_cycle_count = cycle_count
                .checked_mul(block_weight)
                .expect("Cycle count estimate overflow")
                .checked_add(static_cycle_count)
                .expect("Cycle count estimate overflow");
        }
        static_cycle_count
    }
...
impl Shader<'_> {
    ...
    pub fn opt_instr_sched_postpass(&mut self) {
        self.info.static_cycle_count = 0;
        for f in &mut self.functions {
            self.info.static_cycle_count += f.opt_instr_sched_postpass(self.sm);
        }
    }
}
