...
impl LopPass {
    ...
    fn dedup_srcs(&self, op: &mut LogicOp3, srcs: &[Src; 3]) {
        for i in 0..2 {
            for j in (i + 1)..3 {
                if srcs[i].reference == srcs[j].reference {
    ...
    fn try_prop_to_src(&self, ops: &mut [LogicOp3], srcs: &mut [Src; 3], src_idx: usize) {
        loop {
            assert!(srcs[src_idx].is_unmodified());
            let ssa = match &srcs[src_idx].reference {
                SrcRef::SSA(vec) => {
                    assert!(vec.comps() == 1);
                    vec[0]
                }
            ...
            for i in 0..3 {
                ...
                let mut found = false;
                for j in 0..3 {
                    if entry.srcs[i].reference == srcs[j].reference {
                        entry_srcs[i] = j;
                        found = true;
                        break;
                    }
                ...
                loop {
                    ...
                    if next_src == src_idx || src_as_bool(&srcs[next_src]).is_some() {
