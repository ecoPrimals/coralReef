...
/// An SSA value
///
/// Each SSA value represents a single 32-bit or 1-bit (if a predicate) value
...
impl SSARef {
    ...
    /// fit in an SSARef.
    #[inline]
    pub fn new(comps: &[SSAValue]) -> Self {
        Self {
...
mod tests {
    ...
    #[test]
    fn test_ssa_value_gpr() {
        let ssa = SSAValue::new(RegFile::GPR, 1);
        assert_eq!(ssa.idx(), 1);
        assert!(ssa.file().is_gpr());
    }

    #[test]
    fn test_ssa_value_predicate() {
        let ssa = SSAValue::new(RegFile::Pred, 10);
        assert!(ssa.file().is_predicate());
    }
}
