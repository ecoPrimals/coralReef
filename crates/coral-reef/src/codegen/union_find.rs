            ...
            match self.nodes[idx] {
                Node::Child { parent_idx } => {
                    match self.nodes[parent_idx] {
                        ...
                        Node::Root(parent_root) => return (parent_idx, parent_root),
                    }
                }
...
mod tests {
    ...
    impl<'a, X: Copy + Hash + Eq, H: BuildHasher + Default> HeightCalc<'a, X, H> {
        fn new(uf: &'a UnionFind<X, H>) -> Self {
            let mut downward_edges: Vec<Vec<usize>> = uf.nodes.iter().map(|_| Vec::new()).collect();
    ...
    fn test_binary_tree_height() {
        ...
        for current_height in 0..height {
            ...
            assert!(actual_height == current_height || actual_height == current_height + 1);
        }
