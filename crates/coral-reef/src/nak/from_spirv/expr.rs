...
impl<'a, 'b> FuncTranslator<'a, 'b> {
    pub(super) fn ensure_expr(&mut self, handle: Handle<naga::Expression>) -> Result<SSARef, CompileError> {
            ...
            _ => {
                ...
                match inner {
                    naga::TypeInner::Scalar(s) => {
                        s.kind == naga::ScalarKind::Float && s.width == 8
                    }
