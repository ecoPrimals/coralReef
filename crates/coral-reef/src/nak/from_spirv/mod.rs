...
//! Naga IR → NAK IR translator.
//!
//! Translates a `naga::Module` (parsed from SPIR-V or WGSL) into the NAK
//! internal SSA-based IR (`Shader`), which then flows through the standard
//! NAK optimization / legalization / RA / encoding pipeline.
...
pub struct NagaTranslator<'a> {
    sm: &'a ShaderModelInfo,
    module: &'a naga::Module,
}

impl<'a> NagaTranslator<'a> {
    pub fn new(sm: &'a ShaderModelInfo, module: &'a naga::Module) -> Self {
        Self { sm, module }
    }

    /// Translate a compute shader entry point into a NAK `Shader`.
    pub fn translate_compute(
        ...
        let smem_size = self.compute_shared_mem_size();
...
/// Translate a naga Module into a NAK Shader for a compute entry point.
pub fn translate<'a>(
    module: &'a naga::Module,
    sm: &'a ShaderModelInfo,
...
mod tests {
    ...
    fn test_parse_wgsl_valid_minimal_compute() {
        ...
        assert!(result.is_ok(), "valid minimal compute shader should parse: {result:?}");
    ...
    fn test_e2e_compute_with_loop_has_back_edge() {
        ...
        assert!(
            has_back_edge,
            "shader with loop should have CFG back edge"
        );
    }
    ...
    fn test_translate_unary_operations() {
        ...
        assert!(
            has_fadd,
            "abs(-x) uses FAdd with fneg/fabs modifiers"
        );
    }
