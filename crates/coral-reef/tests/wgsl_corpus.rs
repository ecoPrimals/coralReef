...
//! WGSL cross-domain corpus — shaders from all ecosystem compute pipelines.
//!
//! Each shader is compiled for SM70 (Volta) with f64 software lowering enabled.
//! Provenance tracks which domain originated and evolved each shader — several
//! shaders were first written in one domain then absorbed across pipelines.
//!
//! ## Provenance
//!
//! | Domain | Pipeline | Shader path pattern |
//! |--------|----------|---------------------|
//! | Lattice QCD | tensor-dispatch | lattice/shaders/ |
//! | Molecular dynamics | tensor-dispatch | md/shaders/ |
//! | Nuclear physics | tensor-dispatch | physics/shaders/ |
//! | Condensed matter | materials-compute | metalForge/shaders/ |
//! | ML / attention | neural-compute | metalForge/shaders/ |
//! | Hydrology / env | environmental-compute | shaders/ |
//!
//! ## Cross-domain evolution notes
//!
//! - `gelu_f64`, `layer_norm_f64`, `softmax_f64`, `sdpa_scores_f64` — neural-compute
//!   coralForge attention primitives, evolved from tensor-dispatch precision patterns
//!   (FMA, Kahan summation). Used for Evoformer/IPA and bio-statistical DF64 dispatch.
//!
//! - `kl_divergence_f64` — neural-compute statistical primitive. Absorbed by
//!   bio-statistical pipelines for cross-entropy validation and by materials-compute
//!   for Anderson model fitness.
//!
//! - `rk4_parallel` — neural-compute ODE solver (f32). Complex control flow with
//!   loops and temporaries — exercises scheduling and register allocation hard.
//!
//! - `dielectric_mermin_f64`, `bcs_bisection_f64` — tensor-dispatch plasma/nuclear
//!   precision shaders. Demonstrate stable W(z) asymptotic and cancellation-safe
//!   BCS v² formula. Referenced by bio-statistical precision gap analysis.
//!
//! - `su3_gauge_force_f64` — tensor-dispatch lattice QCD. SU(3) matrix ops with
//!   complex traceless anti-Hermitian projection. Heavy f64 with staple sums.
//!
//! - `anderson_lyapunov_f64` — materials-compute condensed matter. xoshiro128** PRNG,
//!   transfer matrix multiplication, uniform buffer bindings. Referenced by
//!   neural-compute for disorder sweep validation.
//!
//! - `stress_virial_f64` — tensor-dispatch MD off-diagonal stress tensor. Used by
//!   bio-statistical pipelines for mechanical property validation.
//!
//! ## Iteration 17 absorptions
//!
//! - CG linear algebra suite (`cg_compute_alpha_f64`, `cg_compute_beta_f64`,
//!   `cg_update_p_f64`, `cg_update_xr_f64`, `complex_dot_re_f64`) — lattice QCD
//!   conjugate gradient kernels. Test scalar + vector CG paths.
//!
//! - Yukawa variants (`yukawa_force_verlet_f64`, `yukawa_force_celllist_indirect_f64`)
//!   — tensor-dispatch MD neighbor-list variants. Exercise `round()`, indirect u32
//!   indexing.
//!
//! - `su3_momentum_update_f64`, `vacf_batch_f64`, `su3_flow_accumulate_f64` —
//!   additional lattice/MD kernels.
//!
//! - `xoshiro128ss` — neural-compute PRNG. Exercises bitwise rotl/shift/xor path.
//!
//! - `hmm_viterbi`, `hmm_backward_log` — neural-compute HMM log-domain. Exercises
//!   argmax, logsumexp, exp/log codegen.
//!
//! - `pairwise_hamming`, `pairwise_jaccard` — neural-compute distance metrics.
//!   Integer diff counting and set-based distance.
//!
//! - `rk45_adaptive` — neural-compute adaptive ODE. `pow()` + scratch buffers.
//!
//! - `matrix_correlation` — neural-compute shared-memory Pearson correlation.
//!
//! - `stencil_cooperation`, `spatial_payoff` — neural-compute game-theory stencils.
//!
//! - `swarm_nn_forward` — neural-compute batch NN forward with integer argmax.
//!
//! ## Iteration 21 absorptions
//!
//! ### Tensor-dispatch domain (9 new + 6 wired)
//!
...
//!
//! ### Neural-compute domain (17 new + 6 wired)
//!
...
//! ### Retired
//!
//! - `local_elementwise_f64` — removed (environmental-compute v0.7.2 retired this
//!   shader; upstream replacement is `batched_elementwise_f64` in tensor-dispatch).
...
// ===========================================================================
// Tensor-dispatch domain — Lattice QCD (precision shaders)
// ===========================================================================
...
// ===========================================================================
// Tensor-dispatch domain — Molecular dynamics
// ===========================================================================
...
// Cross-domain: used by bio-statistical pipelines for mechanical property validation
...
// ===========================================================================
// Tensor-dispatch domain — Nuclear physics (cancellation-safe precision)
// ===========================================================================

// Physics: Mermin dielectric function ε(k,ω) — stable W(z) asymptotic
// Cross-domain: referenced by bio-statistical precision gap analysis
wgsl_compile_test!(
...
// abs_f64 inlined (was preamble-injected by ecosystem ShaderTemplate).
...
// ===========================================================================
// Materials-compute domain — Condensed matter
// ===========================================================================
...
// Cross-domain: referenced by neural-compute for disorder sweep validation
wgsl_compile_test!(corpus_anderson_lyapunov_f64, "anderson_lyapunov_f64.wgsl");

// ===========================================================================
// Neural-compute domain — ML / attention primitives (coralForge)
// Cross-domain: evolved from tensor-dispatch precision patterns (FMA, Kahan);
//   absorbed by bio-statistical DF64 dispatch pipelines
// ===========================================================================
...
// Cross-domain: absorbed by bio-statistical pipelines for cross-entropy
//   validation, materials-compute for Anderson model fitness
...
// Cross-domain: exercises loop scheduling + register pressure
wgsl_compile_test!(corpus_rk4_parallel, "rk4_parallel.wgsl");

// ===========================================================================
// Tensor-dispatch domain — CG linear algebra (Iteration 17 absorption)
// ===========================================================================
...
// ===========================================================================
// Tensor-dispatch domain — Yukawa MD variants (Iteration 17 absorption)
// ===========================================================================
...
// ===========================================================================
// Tensor-dispatch domain — Lattice QCD additional (Iteration 17 absorption)
// ===========================================================================
...
// ===========================================================================
// Neural-compute domain — PRNG / bitwise (Iteration 17 absorption)
// ===========================================================================
...
// ===========================================================================
// Neural-compute domain — HMM / log-domain (Iteration 17 absorption)
// ===========================================================================
...
// ===========================================================================
// Neural-compute domain — Distance / set metrics (Iteration 17 absorption)
// ===========================================================================
...
// ===========================================================================
// Neural-compute domain — Adaptive ODE / stencil (Iteration 17 absorption)
// ===========================================================================
...
// ===========================================================================
// Tensor-dispatch domain — Existing fixtures wired for corpus (Iteration 21)
// ===========================================================================
...
// ===========================================================================
// Neural-compute domain — Existing fixtures wired for corpus (Iteration 21)
// ===========================================================================
...
// ===========================================================================
// Tensor-dispatch domain — New absorption: nuclear/physics (Iteration 21)
// ===========================================================================
...
// ===========================================================================
// Tensor-dispatch domain — New absorption: MD / ESN (Iteration 21)
// ===========================================================================
...
// ===========================================================================
// Tensor-dispatch domain — New absorption: Lattice QCD (Iteration 21)
// ===========================================================================
...
// ===========================================================================
// Neural-compute domain — New absorption: bio/evolution (Iteration 21)
// ===========================================================================
...
// ===========================================================================
// Neural-compute domain — New absorption: coralForge Evoformer (df64, Iteration 21)
