...
//! Local socket JSON-RPC 2.0 server — newline-delimited protocol.
//!
//! Ecosystem primals discover coralReef via a local socket at
...
//! only installed when the socket path includes the ecosystem namespace directory
//! segment (default `biomeos` via [`crate::config::ecosystem_namespace`]).
//!
//! This module serves the same `shader.compile.*` and `health.*` methods as the
//! TCP/HTTP server but over newline-delimited JSON on a local socket.
//!
...
//! ## Platform dispatch
//!
//! On Unix, binds a Unix domain socket via [`crate::local_transport::bind_local`].
//! On non-Unix, [`start_unix_jsonrpc_server`] returns
//! [`std::io::ErrorKind::Unsupported`] — callers degrade gracefully to TCP.
...
/// Start a local socket JSON-RPC server.
///
/// On Unix, binds a Unix domain socket at `path` via
/// [`crate::local_transport::bind_local`]. On non-Unix, returns
/// [`std::io::ErrorKind::Unsupported`] — callers should degrade to TCP.
///
...
/// # Errors
///
/// Returns an error if the socket cannot be bound or local sockets are
/// unsupported on this platform.
pub async fn start_unix_jsonrpc_server(
    path: &Path,
    shutdown_rx: watch::Receiver<()>,
) -> Result<(PathBuf, JoinHandle<()>), std::io::Error> {
    start_local_server_impl(path, shutdown_rx)
}

#[cfg(unix)]
fn start_local_server_impl(
    path: &Path,
            ...
            tokio::select! {
                accept = listener.accept() => {
                    match accept {
                        ...
                        Err(e) => {
                            tracing::warn!("local socket accept error: {e}");
                        }
                    }
                }
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn install_capability_symlink_skips_when_same_as_socket() {
        ...
        let link = crate::local_transport::install_capability_symlink(&socket);
