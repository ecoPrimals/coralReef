...
// --- Shutdown join timeout (test-build env override) ---
...
fn shutdown_join_timeout_mutex_override_roundtrip() {
    ...
    *crate::TEST_SHUTDOWN_JOIN_TIMEOUT_MS_OVERRIDE
    ...
    assert_eq!(
        super::shutdown_join_timeout().as_millis(),
...
fn shutdown_join_timeout_elapsed_message_includes_duration() {
    const NANOS: u32 = 7;
    let msg = super::shutdown_join_timeout_elapsed_message(std::time::Duration::from_nanos(
        u64::from(NANOS),
    ));
    assert!(
...
// --- Full `cmd_server` path (subprocess): listen, discovery, signals ---
...
mod cmd_server_process {
    ...
    fn cmd_server_subprocess_tcp_unix_listen_and_sigterm() {
        ...
        let json: serde_json::Value =
            serde_json::from_str(&contents).expect("parse discovery json");
        ...
        assert!(
            ...
            "Unix JSON-RPC socket should exist at {:?}",
            unix_path
        );
        ...
        let code = loop {
            match child.try_wait().expect("try_wait") {
                ...
                None if Instant::now() >= exit_deadline => {
                    let _ = child.kill();
                    panic!(
                        "child did not exit after SIGTERM within {:?}",
                        CHILD_EXIT_TIMEOUT
                    );
                }
