...
async fn cmd_server_jsonrpc_invalid_bind_returns_general_error() {
    let result = cmd_server("not-a-valid-address", "127.0.0.1:0", None, "127.0.0.1").await;
    assert!(
...
async fn cmd_server_tarpc_invalid_bind_returns_general_error() {
    let result = cmd_server("127.0.0.1:0", "garbage:not-valid", None, "127.0.0.1").await;
    assert!(
        matches!(result, UniBinExit::GeneralError),
...
async fn cmd_server_newline_port_bind_conflict_returns_general_error() {
    ...
    let result = cmd_server("127.0.0.1:0", "127.0.0.1:0", Some(port), "127.0.0.1").await;
    drop(listener);
    assert!(
        matches!(result, UniBinExit::GeneralError),
