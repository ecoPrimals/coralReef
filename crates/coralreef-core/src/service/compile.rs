...
pub fn handle_compile_multi(
    req: super::types::BatchCompileRequest,
) -> Result<super::types::BatchCompileResponse, CompileError> {
    ...
    for (index, job) in req.jobs.into_iter().enumerate() {
        ...
        match result {
            Ok(resp) => {
                success_count += 1;
                results.push(super::types::BatchCompileJobResult {
                    index,
                    label: job.label,
    ...
    Ok(super::types::BatchCompileResponse {
        results,
        success_count,
        total_count,
    })
